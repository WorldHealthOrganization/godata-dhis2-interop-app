...
import { useCreateMetaMappingMutation } from '../../metaMapping'
...
import styles from './MetaDataMappingFormNew.module.css'

const { Form, useForm } = ReactFinalForm

export const META_DATA_MAPPING_FORM_NEW_PATH = '/meta-data-config/new'
...
export const MetaDataMappingFormNew = () => {
    const history = useHistory()
    const [createMetaMapping] = useCreateMetaMappingMutation({
        onComplete: () => history.push(META_DATA_MAPPING_LIST_PATH),
    })
    const onSubmit = values => createMetaMapping(values)
