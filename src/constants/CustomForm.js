import { SingleSelectField, SingleSelectOption, Button, ButtonStrip, ReactFinalForm, TextArea, CenteredContent, CircularLoader, composeValidators, hasValue, string, InputField, TextAreaField } from '@dhis2/ui';
...
import { GODATA_OUTBREAK, GODATA_CASE, GODATA_CONTACT, GODATA_CONTACT_OF_CONTACT, GODATA_ORG_UNIT, GODATA_CUSTOM } from '../constants';
...
import { useCreateCasesConstantMutation, useUpdateCasesConstantMutation } from '.';
...
//              iterate(instanceObject.data[0]);
  ...
  const submitText = initialValues.name ? i18n.t('Save mappings') : i18n.t('Add mappings');
  ...
  const selectedNode = instance => {
    ...
    setValueHolder(instance.namespace), instance.name == 'dhis2' ? setOpen(true) : console.log('wrong element selected'), console.log('select Node with popup:  ' + JSON.stringify(instance.namespace));
    return true;
  };
  ...
  const saveConstant = async godataValue => {
    const godataModelJson = Object.keys(godataModelInput).length != 0 ? JSON.parse(godataModelInput) : {};
    const dhisModelJson = Object.keys(dhisModelInput).length != 0 ? JSON.parse(dhisModelInput) : {};
    const allValues = [];
  ...
  return <Form keepDirtyOnReinitialize onSubmit={onSubmit} initialValues={initialValues}>
      ...
      pristine
                    ...
                    <PageSubHeadline>{i18n.t(objectType + ' mappings setup')}</PageSubHeadline>
                    <FormRow>
                <SingleSelectField 
                        label={i18n.t('Type')} 
                        onChange={({selected}) => setObjectTypeAndMappingName(selected)} 
                        selected={objectType} 
                        dataTest={dataTest('views-gatewayconfigformnew-gatewaytype')}>
                    <SingleSelectOption value={GODATA_OUTBREAK} label={i18n.t(GODATA_OUTBREAK)} />

                    <SingleSelectOption value={GODATA_CASE} label={i18n.t(GODATA_CASE)} />

                    <SingleSelectOption value={GODATA_CONTACT} label={i18n.t(GODATA_CONTACT)} />

                    <SingleSelectOption value={GODATA_CONTACT_OF_CONTACT} label={i18n.t(GODATA_CONTACT_OF_CONTACT)} />
                    
                    <SingleSelectOption value={GODATA_ORG_UNIT} label={i18n.t(GODATA_ORG_UNIT)} />
                </SingleSelectField>
            </FormRow>
            <ButtonStrip>
    ...
    {i18n.t('Select DHIS2 Model')}
                    ...
                    <FormRow>
                        <Field required name='name' //value=''
        //component={InputFieldFF}
        render={() => <InputField id="name" label={i18n.t('Name')} className="" type="text" value={nameInput} //component={InputField}
        onChange={ev => onNameInput(ev.value)} validate={composeValidators(string, hasValue)} required />} validate={composeValidators(string, hasValue)} />
                    </FormRow>



                    <FormRow>
                    <div><ReactJson src={godataValue} onAdd={addNode} onEdit={editNode} onDelete={deleteNode} enableClipboard={selectedNode} theme="apathy:inverted" name={mappingName} displayArrayKey={true} />
            </div>
                    </FormRow>    

                    <Modal open={openGodataModel} onClose={onCloseGodataModel} center>
        <h2>Select Go.Data metadata Model</h2>
        <div><Editor mode='text' value={godataModel} onChange={ev => setGodataModel(ev)} />
            </div>
    </Modal>
    <Modal open={openDhisModel} onClose={onCloseDhisModel} center>
        <h2>Select DHIS2 metadata Model</h2>
        <div><Editor mode='text' value={dhisModel} onChange={ev => setDhisModel(ev)} />
            </div>
    </Modal>

            <Modal open={open} onClose={onCloseModal} center>
        <h2>Select DHIS2 metadata</h2>
        <div><ReactJson src={dhisValue} enableClipboard={copyFromPopup} theme="apathy:inverted" name={'Program'} displayArrayKey={true} />
            </div>
    </Modal>
                    <ButtonStrip>
